...
import {
    IAcceptFriendRequest, IAcknowledgeNewFriend, IAcknowledgeNewFriendRequest,
    IAlreadyFriends, ICancelFriendRequest, ICheckUserExistsForFriend,
    IDeclineAllFriendRequests, IDeclineFriendRequest, IFriendRemoved,
    IFriendRequestAccepted, IFriendRequestAlreadySent, IFriendRequestCanceledOrDeclined,
    IFriendRequestDeclined, IFriendRequestSent, IFriendsList, IFriendsListProps,
    IIncomingFriendRequestExists, ILoadFriends, INewFriendRequest, IRemoveFriend, ISendFriendRequest
} from "./friends.types";
...
export class AcceptFriendRequest extends BasePacket implements IAcceptFriendRequest {
    ...
    static getId() { return -1926185291; }
}
export class AcknowledgeNewFriend extends BasePacket implements IAcknowledgeNewFriend {
    ...
    read(buffer: Buffer) { this.nickname = new BufferReader(buffer).readOptionalString(); }
    write(): Buffer { return new BufferWriter().writeOptionalString(this.nickname).getBuffer(); }
    static getId() { return 1286861380; }
}
export class AcknowledgeNewFriendRequest extends BasePacket implements IAcknowledgeNewFriendRequest {
    ...
    read(buffer: Buffer) { this.nickname = new BufferReader(buffer).readOptionalString(); }
    write(): Buffer { return new BufferWriter().writeOptionalString(this.nickname).getBuffer(); }
    static getId() { return -1041660861; }
}
...
export class FriendRemoved extends BasePacket implements IFriendRemoved {
    ...
    read(buffer: Buffer) { this.nickname = new BufferReader(buffer).readOptionalString(); }
    write(): Buffer { return new BufferWriter().writeOptionalString(this.nickname).getBuffer(); }
    static getId() { return 1716773193; }
}
...
export class FriendRequestAlreadySent extends BasePacket implements IFriendRequestAlreadySent {
    ...
    read(buffer: Buffer) { this.nickname = new BufferReader(buffer).readOptionalString(); }
    write(): Buffer { return new BufferWriter().writeOptionalString(this.nickname).getBuffer(); }
    static getId() { return 2064692768; }
}
...
export class FriendRequestDeclined extends BasePacket implements IFriendRequestDeclined {
    ...
    read(buffer: Buffer) { this.nickname = new BufferReader(buffer).readOptionalString(); }
    write(): Buffer { return new BufferWriter().writeOptionalString(this.nickname).getBuffer(); }
    static getId() { return -1885167992; }
}
export class FriendRequestSent extends BasePacket implements IFriendRequestSent {
    ...
    read(buffer: Buffer) { this.nickname = new BufferReader(buffer).readOptionalString(); }
    write(): Buffer { return new BufferWriter().writeOptionalString(this.nickname).getBuffer(); }
    static getId() { return -1241704092; }
}
export class FriendsList extends BasePacket implements IFriendsList {
    ...
    read(buffer: Buffer) { const r = new BufferReader(buffer); this.acceptedFriends = r.readStringArray(); this.newAcceptedFriends = r.readStringArray(); this.incomingRequests = r.readStringArray(); this.newIncomingRequests = r.readStringArray(); this.outgoingRequests = r.readStringArray(); }
    write(): Buffer { const w = new BufferWriter(); w.writeOptionalStringArray(this.acceptedFriends); w.writeOptionalStringArray(this.newAcceptedFriends); w.writeOptionalStringArray(this.incomingRequests); w.writeOptionalStringArray(this.newIncomingRequests); w.writeOptionalStringArray(this.outgoingRequests); return w.getBuffer(); }
    static getId() { return 1422563374; }
}
export class IncomingFriendRequestExists extends BasePacket implements IIncomingFriendRequestExists {
    ...
    read(buffer: Buffer) { this.nickname = new BufferReader(buffer).readOptionalString(); }
    write(): Buffer { return new BufferWriter().writeOptionalString(this.nickname).getBuffer(); }
    static getId() { return -1258754138; }
}
export class LoadFriends extends BasePacket implements ILoadFriends {
    unknown: boolean = false;
    read(buffer: Buffer) { this.unknown = new BufferReader(buffer).readUInt8() === 1; }
    write(): Buffer { return new BufferWriter().writeUInt8(this.unknown ? 1 : 0).getBuffer(); }
    static getId() { return -731115522; }
}
